...
The shapefiles for the data come from [Pennsylvania Spatial Data Access](http://www.pasda.psu.edu/uci/DataSummary.aspx?dataset=1214).
...
Next we have to clean up the shapefile geometry:
```{r}
shapefile <- shapefile %>% 
  st_make_valid() %>% 
  clean_names() %>% 
...
Then, join the shapefiles geometry and parcel data:
```{r}
...
parcel_geometry_cleaned <- bind_cols(parcel_data, centroids) %>% 
  ...
  filter(!is.na(yearblt_asmt),
         yearblt_asmt > 1000, yearblt_asmt <= 1900) %>% 
